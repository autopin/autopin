/*
 * compact_ia64_smpl.c - compact output sampling module for perfmon v2.0
 *
...
#include <perfmon/perfmon_default_smpl.h> /* compat mode ONLY */
...
compact_process_samples(pfmon_sdesc_t *sdesc)
{
	...
	while(count--) {
		if (skip == 0)
			ret = fprintf(fp, "%-8lu %8d %8d %2d 0x%llx 0x%016"PRIx64" %3u %5u %16"PRId64" ",
					entry,
					ent->pid,
					ent->tgid,
					ent->cpu,
...
compact_print_header(pfmon_sdesc_t *sdesc)
{
	...
	fprintf(fp, "# description of columns:\n"
		    ...
		    "#\tfollowed by optional samples PMD values in increasing index order\n");
	return 0;
}

pfmon_smpl_module_t compact_old_smpl_module;
static void compact_initialize_mask(void)
{
	pfmon_bitmask_setall(compact_old_smpl_module.pmu_mask);
}
